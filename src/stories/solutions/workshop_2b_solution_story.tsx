...
storiesOf('Development/d3-workshop/Solutions/2b', module).addDecorator(getStory => {
  const style = {
...
function drawPoints(element: Element | null | undefined, props: Props) {
  const points = select(element).selectAll('circle').data(props.points);
  const xScale = scaleBand().range([0, props.width]).domain(props.points);
...
function drawBars(element: Element | null | undefined, props: Props) {
  const points = select(element).selectAll('rect').data(props.points);
  const xScale = scaleBand().range([0, props.width]).domain(props.points).padding(0.1);
  points.enter().append('rect').attr('fill', 'black').attr('height', BAR_HEIGHT).attr('width', xScale.bandwidth()).attr('y', props.height / 2).attr('x', d => xScale(d));
  const texts = select(element).selectAll('text').data(props.points);
  texts.enter().append('text').attr('text-anchor', 'middle').attr('y', props.height / 2 - BAR_HEIGHT).attr('x', d => xScale(d)).text(d => d);
}