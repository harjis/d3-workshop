// @ts-expect-error shut the fuck up, Flow!
 ...
 * */
storiesOf('Development/d3-workshop/Solutions/2extra', module).addDecorator(getStory => {
  const style = {
...
function Boxes(props: Props) {
  ...
  const toggleBox = box => setBoxes(selectedBoxes.includes(box) ? [...new Set(selectedBoxes.filter(selectedBox => selectedBox !== box))] : [...new Set(selectedBoxes.concat(box))]);
...
function initColorBucket() {
  ...
  const removeNotExisting = (currentIds, newIds) => currentIds.map(currentId => {
